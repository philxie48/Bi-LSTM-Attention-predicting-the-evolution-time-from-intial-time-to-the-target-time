...
class CombinedMAEAutocorrLoss(nn.Module):
    """
    Combined loss function with MAE and Autocorrelation components.
    
    This combines pointwise MAE loss with autocorrelation loss to
    preserve both absolute values and temporal patterns.
    """
    def __init__(self, alpha=0.5, max_lag=10):
        ...
        Args:
            alpha (float): Weight for MAE loss (1-alpha is weight for autocorrelation loss)
            max_lag (int): Maximum lag for autocorrelation calculation
        """
        super(CombinedMAEAutocorrLoss, self).__init__()
        self.alpha = alpha
    ...
    def forward(self, y_pred, y_true):
        ...
        Args:
            y_pred (torch.Tensor): Predicted sequence
        ...
        # Combine losses with equal weighting (50% each)
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        if hasattr(model, 'mixed_precision') and model.mixed_precision:
            with autocast():
                ...
                loss, mae_loss, autocorr_loss = criterion(outputs, targets)
        ...
        total_autocorr_loss += autocorr_loss.item()
        
        if batch_idx % 50 == 0:
            # Print current learning rate
            current_lr = optimizer.param_groups[0]['lr']
            print(f"Epoch {epoch+1}/{total_epochs}, Batch {batch_idx}/{len(train_loader)}, Loss: {loss.item():.6f}, MAE: {mae_loss.item():.6f}, Autocorr: {autocorr_loss.item():.6f}, LR: {current_lr:.6f}")
    
    avg_loss = total_loss / len(train_loader)
    avg_mae_loss = total_mae_loss / len(train_loader)
    avg_autocorr_loss = total_autocorr_loss / len(train_loader)
    print(f"Epoch {epoch+1}/{total_epochs} completed in {time.time() - start_time:.2f}s, Avg Loss: {avg_loss:.6f}, Avg MAE: {avg_mae_loss:.6f}, Avg Autocorr: {avg_autocorr_loss:.6f}")
    
    return avg_loss, avg_mae_loss, avg_autocorr_loss
...
def validate(model, val_loader, criterion, device):
    ...
    avg_autocorr_loss = total_autocorr_loss / len(val_loader)
    print(f"Validation Loss: {avg_loss:.6f}, MAE: {avg_mae_loss:.6f}, Autocorr: {avg_autocorr_loss:.6f}")
...
def test(model, test_loader, criterion, device):
    ...
    avg_autocorr_loss = total_autocorr_loss / len(test_loader)
    print(f"Test Loss: {avg_loss:.6f}, MAE: {avg_mae_loss:.6f}, Autocorr: {avg_autocorr_loss:.6f}")
    
    return avg_loss, avg_mae_loss, avg_autocorr_loss

def train_model(args):
    """Train the model with combined MAE and autocorrelation loss."""
    # Set random seed for reproducibility
    set_seed(args.seed)
    ...
    # Define loss function (Combined MAE and Autocorrelation Loss)
    criterion = CombinedMAEAutocorrLoss(alpha=0.5, max_lag=args.max_lag)
    ...
    for epoch in range(args.epochs):
        ...
        # Train for one epoch
        train_loss, train_mae_loss, train_autocorr_loss = train_epoch(
        ...
        # Validate the model
        val_loss, val_mae_loss, val_autocorr_loss = validate(model, val_loader, criterion, device)
        
        # Test the model
        test_loss, test_mae_loss, test_autocorr_loss = test(model, test_loader, criterion, device)
        ...
        log['train_mae_loss'].append(train_mae_loss)
        log['train_autocorr_loss'].append(train_autocorr_loss)
        log['val_loss'].append(val_loss)
        log['val_mae_loss'].append(val_mae_loss)
        log['val_autocorr_loss'].append(val_autocorr_loss)
        log['test_loss'].append(test_loss)
        log['test_mae_loss'].append(test_mae_loss)
        log['test_autocorr_loss'].append(test_autocorr_loss)
...
def export_metrics_to_csv(log, output_dir):
    metrics_df = pd.DataFrame({
        ...
        'Train MAE Loss': log['train_mae_loss'],
        'Train Autocorr Loss': log['train_autocorr_loss'],
        'Validation Loss': log['val_loss'],
        'Validation MAE Loss': log['val_mae_loss'],
        'Validation Autocorr Loss': log['val_autocorr_loss'],
        'Test Loss': log['test_loss'],
        'Test MAE Loss': log['test_mae_loss'],
        'Test Autocorr Loss': log['test_autocorr_loss'],
        'Learning Rate': log['lr']
    })
    metrics_df.to_csv(os.path.join(output_dir, 'training_metrics.csv'), index=False)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train a sequence generator model with combined MAE and autocorrelation loss")
    ...
    # Output parameters
    parser.add_argument("--output_dir", type=str, default="D:/neural network/50mae_50auto", help="Directory to save model checkpoints")
